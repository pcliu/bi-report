...
def generate_complete_pdf_report():
    ...
    stats_data = [
        ['指标', '数值'],
        ['总记录数', f"{total_records.iloc[0]['count']:,}" if not total_records.empty else "N/A"],
        ['用户总数', f"{total_users.iloc[0]['count']:,}" if not total_users.empty else "N/A"],
        ['总流量', f"{total_traffic.iloc[0]['total'] / (1024*1024*1024):.2f} GB" if not total_traffic.empty else "N/A"],
        ['应用大类数', f"{app_categories.iloc[0]['count']:,}" if not app_categories.empty else "N/A"],
    ]
